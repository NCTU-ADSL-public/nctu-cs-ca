        ...
        >
            {items.map((item,key) =>
                ...
                >
                    ...
                    <br/>
                    {(item.reason==="notCS") ? <div>此為外系課程，必須申請過抵免才能算通過。</div> : <div> </div>}
                    {(item.reason==="free1") ? <div>您已申請過抵免了。</div> : <div> </div>}
                    {(item.reason==="free2") ? <div>免修課程。</div> : <div> </div>}
                    {(item.reason==="english") ? <div>此為抵免英文檢定考試的課程。</div> : <div></div>}
                    {(item.reason==="now") ? <div>當期課程。</div> : <div></div>}
                    {(item.reason==="now" && item.complete) ? <div>已修過這堂課，目前正重複修課中。</div> : <div></div>}
                </PopoverButton>
            )}
        </div>
    )
