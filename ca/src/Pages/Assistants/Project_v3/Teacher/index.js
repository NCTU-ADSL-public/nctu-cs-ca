...
class index extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      filter_project: [false, false, false],
      open_filter: false,
  ...
  toggleStatusFilter = target => {
    this.setState({ filter_status: this.state.filter_status.map((value, index) => target === index ? !value : value), page: 0 })
  }
  toggleProjectFilter = target => {
    this.setState({ filter_project: this.state.filter_project.map((value, index) => target === index ? !value : value), page: 0 })
  }
        ...
        <div className='row' >
          <div className='col-md-4 col-lg-4 col-xs-12' style={{ display: 'flex' }} >
            ...
            <Dialog onClose={() => this.setState({ open_filter: false })} open={open_filter} >
              ...
              <DialogTitle><div style={{ fontSize: '25px' }} >專題級別</div></DialogTitle>
              <div style={{ display: 'flex' }}>
                {
                                        ...
                                        project.students.map(student => (
                                          !filter_project.reduce((haveTrue, value) => haveTrue || value, false) || filter_project[parseInt(student.first_second) - 1] ?
                                            ...
                                            <Chip
                                              ...
                                              deleteIcon={student.add_status === "0" ? student.first_second === "3" ? <Warning style={{ fontSize: 30, color: grey[300] }} /> : <Add style={{ fontSize: 30, color: grey[300] }} /> : <DoneIcon style={{ fontSize: 30, color: grey[300] }} />}
                                              onDelete={() => console.log("You Find It")}
                                              avatar={<Avatar style={{ fontSize: 20, background: grey[100], color: grey[300] }}>{STUDENT_STATUS_CN[parseInt(student.status, 10)]}</Avatar>}
                                            />
                                        ))
                                      }
                                    </div>
