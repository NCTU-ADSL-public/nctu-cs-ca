...
class VerifyDownload extends React.Component {
    ...
    switch (i) {
      case 2:
        return (Verify.formList.filter(e => (e.type === 2 && `${e.apply_year}-${e.apply_semester}` === getSemester()))
        .map(e => {
          return ({
            type: '學分抵免',
            sid: e.sid,
      ...
      case 3:
        return (Verify.formList.filter(e => (e.type === 3 && `${e.apply_year}-${e.apply_semester}` === getSemester()))
        .map(e => {
          return ({
            type: '課程免修',
            sid: e.sid,
      ...
      case 0:
        return Verify.formList.filter(e => e.type === 0 && `${e.apply_year}-${e.apply_semester}` === getSemester())
      .map(e => {
        return ({
          type: '本系必修課程抵免',
          sid: e.sid,
      ...
      case 1:
        return Verify.formList.filter(e => e.type === 1 && `${e.apply_year}-${e.apply_semester}` === getSemester())
      .map(e => {
        return ({
          type: '英授專業課程抵免',
          sid: e.sid,
