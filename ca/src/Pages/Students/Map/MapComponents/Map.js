...
import { addTodo, reviseselectvalue } from '../actions/index'
import App from './App'
import { GitEdgePre, GitEdgeSug } from './NodeExtension'
...
class Map extends React.Component {
  constructor (props) {
    ...
    this.SavingCourseData = this.SavingCourseData.bind(this)
    this.state = {
      value: 1,
      data: [],
      datar: [],
        ...
        if (i !== 0) {
          if (this.props.data[i - 1].cos_cname !== this.props.data[i].cos_cname) {
            if (semflag !== this.props.data[i].semester) {
              ...
              nx += 220
              datai.push({ 'id': (id++).toString(),
                'label': semarr[semcount],
                'position': { 'x': nx, 'y': 0 },
                'size': {
                  'width': 80,
                  'height': 50
            ...
            datai.push({ 'id': (id).toString(),
              'label': this.props.data[i].cos_cname,
              'position': { 'x': nx, 'y': ny },
              'size': {
                'width': 80,
                'height': 50
